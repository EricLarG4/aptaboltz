...
# --- Miscellaneous aptamers ---

# dictionary of aptamer names and ligand CDC code
LGD_DICT = {"18th5Cl1": "PCI", "Caff209-3bp": "CFF", "Trp94-2bp": "TRP", "His2": "HSM", "MN4": "QI9"}
...
def process_experiment(model, experiment):
    ...
    for file in sdf_files:
        ...
        mol = chem.SDMolSupplier(f"{experiment_path}{file}", removeHs=False, sanitize=True)[0]
        if mol is None:
            mol = chem.SDMolSupplier(f"{experiment_path}{file}", removeHs=False, sanitize=False)[0]
        else:
