...
PLACEHOLDER_MAP = {
    "__JOB_NAME__": None,
    "__EXPERIMENT__": None,
    "__ARRAY_WITH_CONCURRENCY__": None,
    "__SCRATCHDIR__": None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scratchdir",
        required=True,
        help="Relative path under $SLURM_SUBMIT_DIR to the scratch directory "
        "containing pmemd/ and leap/.",
    )
    parser.add_argument(
        "--gpu",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run array tasks sequentially (%1 throttle). Default: false",
    )
...
def main() -> None:
    ...
    array_str = args.array
    if args.sequential:
        if "%" in array_str:
            print("Warning: --array already contains a concurrency throttle "
                  f"('%' found). Ignoring --sequential.")
        else:
            array_str += "%1"

    script_dir = pathlib.Path(__file__).resolve().parent
    template_path = script_dir.parent / "slurm" / "PrepAndMin.slurm"
    ...
    replacements = {
        ...
        "__ARRAY_WITH_CONCURRENCY__": array_str,
    ...
    output_dir = (script_dir.parent / "slurm").resolve()
